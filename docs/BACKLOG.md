...
| A6 | **`TOKEN_GUARD_SID` nunca é setado por caminho de produção**, só por `test/daemon-adapters-parity.test.cjs` (isolamento de teste). `defaultEndpoint()` no Windows (`adapters/daemon-server.cjs`) cai em `process.env.TOKEN_GUARD_SID \|\| String(process.pid)` — como cada hook roda num processo curto com PID próprio, dois processos de hook distintos nunca convergem pro mesmo named pipe sem essa var setada de fora. Efeito prático: o start-on-demand de F5 (`lib/daemon-client.cjs`) funciona corretamente e passa nos testes (endpoint injetado), mas em produção real no Windows sem um mecanismo que fixe `TOKEN_GUARD_SID` (ex.: escrito num arquivo de sessão pelo instalador, ou derivado do diretório do projeto), cada chamada de hook spawna um daemon novo em vez de reusar um já de pé — o daemon nunca fica "quente" entre hooks. Documentado como DECISION em F3 (não corrigido ali, adiado pro plano original pra F4); F4 (lock/singleton) não tocou em cálculo de endpoint/SID; F5 (start-on-demand) tampouco — precisa de um dono explícito antes do release (F7 autostart é candidato natural, mas hoje não cobre isso) | investigação F5 daemon-único (sessão 2026-09-19) | M |
