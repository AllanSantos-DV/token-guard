...
function renderAdvice(advices, pad = 26) {
  ...
  const restam = advices.length - Math.min(advices.length, 8);
  if (restam > 0) L.push(`  … +${restam} recomendação(ões) restantes — rode com --json para a lista completa`);
  L.push('');
  return L;
}
...
function renderText(result) {
  ...
  const cpt = totals.charsPerToken > 0 ? totals.charsPerToken : 4;
  const W = 74;
  const out = [];
  ...
  if (okServers.length) {
    ...
    for (const s of okServers) {
      ...
      p(`  ${s.name.slice(0, 26).padEnd(26)} ${String(s.tools.length).padStart(7)} ${String(n(t)).padStart(10)}   ${(s.schemaChars / cpt / totals.contextWindow).toFixed(3).replace('.', ',')}`);
