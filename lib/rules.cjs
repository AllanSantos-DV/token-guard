...
function ruleNoisePath(ctx) {
  ...
  for (const t of targets) {
    ...
    // dentro de build/dependência, não o nome dos ancestrais do repositório.
    // (Sem isto, um repo clonado em D:\build\meu-projeto ficava 100% bloqueado.)
    let probe = t;
    if (root) {
      try {
        const rel = path.relative(root, path.isAbsolute(t) ? t : path.resolve(root, t));
        if (rel && !rel.startsWith('..')) probe = rel;
...
function ruleShellDump(ctx) {
  ...
  return {
    ...
    reason:
      ...
      `(3) for a whole-tree survey, run the audit — \`npx token-guard audit\` (repo installs: ` +
      ...
      `Para um panorama do repositório inteiro, rode \`npx token-guard audit\`.`,
  };
}
