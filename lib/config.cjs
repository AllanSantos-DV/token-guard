...
/**
 * Arquivos que `load()` consulta para este diretório, na ordem de precedência
 * (globais primeiro, repositório por último). Os caminhos globais entram mesmo
 * quando ausentes: quem usa esta lista como chave de invalidação precisa
 * enxergar a CRIAÇÃO de uma config, não só a edição.
 * @returns {string[]}
 */
function configFilesInEffect(startDir) {
  const files = GLOBAL_DIRS.map((seg) => path.join(os.homedir(), ...seg, CONFIG_NAME));
  const repoFile = findConfigFile(startDir);
  if (repoFile) files.push(repoFile);
  return files;
}
...
module.exports = {
  load, loadUncached, clearMemo, configFilesInEffect, repoStats,
  DEFAULTS, GLOBAL_DIRS, CONFIG_NAME,
};
